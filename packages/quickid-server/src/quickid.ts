...
const DEFAULT_POLL_MAX_DURATION_MS = 300_000; // 5 minutes
...
class SessionsNamespace {
   ...
   * ```
   */
  async poll(sessionId: string, options?: PollOptions): Promise<SessionResult> {
    const intervalMs = options?.intervalMs ?? DEFAULT_POLL_INTERVAL_MS;
    const maxDurationMs =
      options?.maxDurationMs ?? DEFAULT_POLL_MAX_DURATION_MS;
...
export class QuickIDServer {
  ...
  constructor(config: QuickIDServerConfig) {
    // Validate API key
    if (!config.apiKey) {
      throw new QuickIDValidationError("apiKey is required", "apiKey");
    }
