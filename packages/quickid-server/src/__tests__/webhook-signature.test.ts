...
describe("Webhook Signature", () => {
	...
	const PAYLOAD = '{"id":"evt_123","type":"identity.verification_session.verified"}';
	...
	describe("verifySignature", () => {
		...
		it("rejects tampered payload", () => {
			...
			expect(verifySignature(SECRET, '{"tampered":"payload"}', header)).toBe(false);
		});
	...
	describe("constructEvent", () => {
		...
		it("throws QuickIDSignatureVerificationError for invalid signature", () => {
			const header = generateSignatureHeader("wrong_secret", PAYLOAD, TIMESTAMP);
			const parse = (body: string) => JSON.parse(body);
