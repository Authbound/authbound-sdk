...
export interface PensionDemoOption {
  slug: string;
  title: string;
}
...
<body>
  ...
  <script>
    ...
    function setWalletLink(container, href, label) {
    ...
    issueButton.addEventListener('click', async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          issueError.textContent = errorMessage(body);
          return;
        }

        const offerUri = body.offer?.offerUri || body.offerUri;
        if (!offerUri) throw new Error('Offer response did not include a wallet URI');
        const typeCode = body.credential?.credential?.credentialSubject?.Pension?.typeCode || '';
        await drawQr(issueCanvas, offerUri);
        issueQrStage.hidden = false;
        setWalletLink(issueOffer, offerUri, 'Avaa wallet-offer');
        issueMeta.hidden = false;
        issueMeta.textContent = 'Offer ' + body.offer.id + (typeCode ? ' · ' + typeCode : '');
    ...
    async function pollVerificationStatus() {
      ...
      const body = await response.json();
      if (!response.ok) {
        showVerifyError(errorMessage(body));
        stopPolling();
        resetVerifyUi();
        return;
      }
    ...
    verifyButton.addEventListener('click', async () => {
      ...
      try {
        ...
        if (authorizationRequestUrl) {
          await drawQr(verifyCanvas, authorizationRequestUrl);
          verifyQrStage.hidden = false;
          setWalletLink(verifyOffer, authorizationRequestUrl, 'Avaa vahvistuspyyntö');
        }
        verifyBadge.hidden = false;
